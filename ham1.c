/* LANGEVIN DYNAMICS FOR PARTICLES COMMUNICATING VIA OPTIMAL-FIELD COMMUNICATION RULES (Hamiltonian approximation).
...
int main(int argc, char *argv[])
{
  ...
  for(step = start; step < number_step; step++){ 
        ...
        fp1=0.0; //initialize fp1
		fp2x=0.0; //initialize fp2x
		fp2y=0.0; //initialize fp2y
        
        for (i1=0; i1<nparticles; i1++){
		
		fptx[i1]=0.0; //initialize fptx
		fpty[i1]=0.0; //initialize fpty
	}
        ...
        /*Force calculation*/
         for(i1=0; i1< nparticles; i1++){ 
			 for(j1=0; j1< nparticles; j1++){ 
	             ...
	             if(dist>0.0001){
	             fp1=exp(-kappa*dist)/(dist);
	             fp2x=-exp(-kappa*dist)*(kappa*dist+1)*(xij)/pow(dist,3);
	             fp2y=-exp(-kappa*dist)*(kappa*dist+1)*(yij)/pow(dist,3);
	             fptx[i1]=fptx[i1]-lamb*(fp1-Tcom)*fp2x; //force according to Eq.[4]
			     fpty[i1]=fpty[i1]-lamb*(fp1-Tcom)*fp2y;
	             }
				}
			 }
...
void streamfile(int step)
{
  ...
  char folder[81];	
  char file[81];
  sprintf(p_dat, "P%d_T%.3f_Lx%d_Np%d_Dt%.5f/",seir,Tcom,Lx,nparticles,Dt);  //Name of folder
  strcpy(folder, p_dat);
